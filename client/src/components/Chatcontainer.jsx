...
{/* //header */}
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-auto scrollbar-hide p-3 pb-6'>
  ...
  {messages?.filter(msg => msg && msg.senderId).map((msg, index) => (
    ...
    >
      ...
      {msg.image ? (
        ...
        <p
          className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg overflow-hidden mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === authUser?._id ? 'rounded-br-none' : 'rounded-bl-none'
          }`}
        >
          {msg.text}
        </p>
